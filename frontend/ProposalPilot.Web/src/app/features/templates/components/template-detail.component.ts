...
export class TemplateDetailComponent implements OnInit {
  ...
  useTemplate(): void {
    if (this.template) {
      // TODO: Navigate to proposal creation with template selected
      this.router.navigate(['/proposals/new'], {
        queryParams: { templateId: this.template.id }
      });
    }
  }
  ...
  duplicateTemplate(): void {
    ...
    this.templateService.duplicateTemplate(this.template.id).subscribe({
      next: (newTemplate) => {
